...
+ .emph.cerulean[The Rural Shrink Smart EDA dashboard incorporates two major components]: 
    + Data Visualization Interface component with Guided Discovery Learning (GDL) principles, which engages the town leaders. 
    + Exploratory Data Analysis component utilizing statistical clustering methods, which allow town leaders the opportunity explore new elements to add or remove.
...
.large.emph[
+ Paralllel Coordinate Plot with objective measures related to the QoL Survey
]
...
+ Our feedback from users has two components:
   ...
   + User behaviors being record in the background:
      ...
      + e.g. "I'm having a hard time understanding the PCP information", we will see that users are not spending any time on the PCP plot.
---
class:primary
