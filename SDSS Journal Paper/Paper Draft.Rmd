...
Secondly in our workflow, we will look into using Unsupervised Clustering Methods to help our towns make decisions that are informed in comparison to similar towns, for budget size, population size and location.Our research implemented unsupervised statistical methods, such as K-means, Principal Component Analysis (PCA), and Hierarchical Clustering, to determine the next five to ten similar towns, based on distances and services available. We continue to use clustering methods to identify the towns that will be best compared when the analyst interacts with our dashboard. We will take into account that towns will make sure to help towns not make comparisons to towns that may be a bit out of reach and not similar in make-up. We will discuss this more in detail in the discussion, our original goal at using the clustering methods to determine similar towns actually led us to using the methods to determine the data quality of our open-source data.
...
\begin{center}
\includegraphics[width=.3\textwidth]{Version1}
\includegraphics[width=.3\textwidth]{Version2}
\includegraphics[width=.3\textwidth]{Version3}
\includegraphics[width=.3\textwidth]{Version4}
\end{center}
