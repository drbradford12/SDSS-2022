...
In our research, it was necessary to use the visual orientation of dashboards and the framework of Guided Discovery Learning (GDL) to make sure that the town analyst can understand the visual orientation easily and guide them to a discovery about possible changes in the dashboard. 
...
A bit of background on Guided Discovery, which is defined as an environment where "students" are actively discovering knowledge, DeDonno states that we use GDL in our daily lives to make decisions by hints, feedback, and helpful information \cite{dedonno}.
%\begin{quotation}
